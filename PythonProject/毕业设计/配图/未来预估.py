...
projections = xr.DataArray(projections, coords=[['SSP126', 'SSP245', 'SSP585'], Model_Name, [str(i) for i in range(eval(time[0]), eval(time[1]) + 1)]], dims=['ssp', 'model', 'time'])
...
sns.move_legend(fig, "upper left", title=None, frameon=False, bbox_to_anchor=(.15, .95))
...
x = np.arange(0, 78, 1)
ax.set_xlim(0, 78)
ax.set_xticks([0]+np.arange(4, 75, 5).tolist()+[78])
...
'''plt.axvline(x=19, color='gray', linestyle='--', linewidth=0.5)
plt.fill_between(x[0:20], 0, 62, facecolor='blue', alpha=0.3)
plt.axvline(x=39, color='gray', linestyle='--', linewidth=0.5)
plt.fill_between(x[19:40], 0, 62, facecolor='green', alpha=0.3)
plt.axvline(x=59, color='gray', linestyle='--', linewidth=0.5)
plt.axvline(x=78, color='gray', linestyle='--', linewidth=0.5)
plt.fill_between(x[59:79], 0, 62, facecolor='orange', alpha=0.3)'''
plt.xlabel('Year')
plt.ylabel('EHDs')
