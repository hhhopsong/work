...
ax1.plot(lon_, lat_, color='blue', linewidth=2, linestyle='-',transform=ccrs.PlateCarree(central_longitude=0))
...
lon_ = [i for i in np.linspace(lon1, lon2, smoonth)] + [i for i in np.linspace(lon1, lon2, smoonth)][::-1] + [lon1]
lat_ = [lat1 for i in range(smoonth)] + [lat2 for i in range(smoonth)] + [lat1]
ax2.plot(lon_, lat_, color='blue', linewidth=2, linestyle='-',transform=ccrs.PlateCarree())

ax2.add_feature(cfeature.LAND.with_scale('110m'), color='lightgray')# 添加陆地并且陆地部分全部填充成浅灰色
...
plt.savefig(r'C:\Users\10574\OneDrive\File\Graduation Thesis\论文配图\预测因子2mT.png', dpi=1000, bbox_inches='tight')
plt.show()
