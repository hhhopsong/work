...
p_t2m78_2 = np.zeros((len(lat_T2m), len(lon_T2m)))
p_t2m78_2.fill(np.nan)
p_t2m78_2[np.abs(t_t2m78_2) > t_critical] = 1

p_sst78_2 = np.zeros((len(lat_sst), len(lon_sst)))
p_sst78_2.fill(np.nan)
p_sst78_2[np.abs(t_sst78_2) > t_critical] = 1

p_pre78_2 = np.zeros((len(lat_pre), len(lon_pre)))
p_pre78_2.fill(np.nan)
p_pre78_2[np.abs(t_pre78_2) > t_critical] = 1

p_z50078_2 = np.zeros((len(lat_uvz), len(lon_uvz)))
p_z50078_2.fill(np.nan)
p_z50078_2[np.abs(t_z50078_2) > t_critical] = 1
...
# 去除180白线
r_t2m78_2, a1_lon = add_cyclic_point(r_t2m78_2, coord=lon_T2m)
r_sst78_2, a1_sst_lon = add_cyclic_point(r_sst78_2, coord=lon_sst)
p_t2m78_2, a1_lon_p = add_cyclic_point(p_t2m78_2, coord=lon_T2m)
p_sst78_2, a1_sst_lon_p = add_cyclic_point(p_sst78_2, coord=lon_sst)
print('开始绘制地图1')
ax1.set_title('(a)Corr. PC2 & JA SST,2mT', fontsize=28, loc='left')
a1 = ax1.contourf(a1_lon, lat_T2m, r_t2m78_2, cmap=cmaps.GMT_polar, levels=level1, extend='both',
                  transform=ccrs.PlateCarree(central_longitude=0))
a1_sst = ax1.contourf(a1_sst_lon, lat_sst, r_sst78_2, cmap=cmaps.GMT_polar, levels=level1, extend='both',
                      transform=ccrs.PlateCarree(central_longitude=0))
# 通过打点显示出通过显著性检验的区域
a1_p = ax1.contourf(a1_lon_p, lat_T2m, p_t2m78_2, levels=[0, 1], transform=ccrs.PlateCarree(central_longitude=0),
                    hatches=['..', None], colors="none", add_colorbar=False, zorder=5)
a1_sst_p = ax1.contourf(a1_sst_lon_p, lat_sst, p_sst78_2, levels=[0, 1], transform=ccrs.PlateCarree(central_longitude=0),
                        hatches=['..', None], colors="none", add_colorbar=False, zorder=5)
ax1.add_geometries(Reader(r'D:\CODES\Python\PythonProject\map\shp\Yangtze-River middle and lower\Yangtze-River middle and lower.shp').geometries(), ccrs.PlateCarree(), facecolor='none',edgecolor='black',linewidth=2)
...
# 筛选出大于0.35的风场格点
U1threshold = np.where(np.abs(r_u85078_2) > 0.35, 1, 0)
V1threshold = np.where(np.abs(r_v85078_2) > 0.35, 1, 0)
UV1threshold = np.where((U1threshold + V1threshold) >= 1, 1, 0)
r_u85078_2 = np.where(UV1threshold == 1, r_u85078_2, np.nan)
r_v85078_2 = np.where(UV1threshold == 1, r_v85078_2, np.nan)
# 去除180白线
r_pre78, a2_lon = add_cyclic_point(r_pre78_2, coord=lon_pre)
r_u85078, a2_uv_lon = add_cyclic_point(r_u85078_2, coord=lon_uvz)
r_v85078, a2_uv_lon = add_cyclic_point(r_v85078_2, coord=lon_uvz)
p_pre78, a2_lon_p = add_cyclic_point(p_pre78_2, coord=lon_pre)
ax2.set_title('(b)Corr. PC2 & JA PRE,850UV', fontsize=28, loc='left')
...
a2_p = ax2.contourf(a2_lon_p, lat_pre, p_pre78, levels=[0, 1], transform=ccrs.PlateCarree(central_longitude=0),
                    hatches=['..', None], colors="none", add_colorbar=False, zorder=5)
ax2.text(137.5, 51.6, 'A', fontsize=28, fontweight='bold', color='blue', zorder=20, transform=ccrs.PlateCarree(central_longitude=0))
ax2.text(130, 31, 'C', fontsize=28, fontweight='bold', color='red', zorder=20, transform=ccrs.PlateCarree(central_longitude=0))
ax2.text(115, 21, 'A', fontsize=28, fontweight='bold', color='blue', zorder=20, transform=ccrs.PlateCarree(central_longitude=0))
ax2.add_geometries(Reader(r'D:\CODES\Python\PythonProject\map\shp\Yangtze-River middle and lower\Yangtze-River middle and lower.shp').geometries(), ccrs.PlateCarree(), facecolor='none',edgecolor='black',linewidth=2)
...
r_z50078, a3_lon = add_cyclic_point(r_z50078_2, coord=lon_uvz)
p_z50078, a3_lon_p = add_cyclic_point(p_z50078_2, coord=lon_uvz)
ax3.set_title('(c)Corr. PC2 & JA Z500', fontsize=28, loc='left')
...
a3_p = ax3.contourf(a3_lon_p, lat_uvz, p_z50078, levels=[0, 1], transform=ccrs.PlateCarree(central_longitude=0),
                    hatches=['..', None], colors="none", add_colorbar=False, zorder=5)
ax3.add_geometries(Reader(r'D:\CODES\Python\PythonProject\map\shp\Yangtze-River middle and lower\Yangtze-River middle and lower.shp').geometries(), ccrs.PlateCarree(), facecolor='none',edgecolor='black',linewidth=2)
...
plt.savefig(r'C:\Users\10574\OneDrive\File\Graduation Thesis\论文配图\图5.png', dpi=1000, bbox_inches='tight')
plt.show()
