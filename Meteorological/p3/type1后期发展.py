...
#———————————————————因子2——————————————————
sst_2_0 = sst.sel(time=sst['time.month'].isin([12])).groupby('time.year').mean('time').transpose('year', 'lat', 'lon').shift(year=1).sel(year=slice(f'{TR_time[0]+1}', f'{TR_time[1]}'))
sst_2_1 = sst.sel(time=sst['time.month'].isin([5])).groupby('time.year').mean('time').transpose('year', 'lat', 'lon').sel(year=slice(f'{TR_time[0]+1}', f'{TR_time[1]}'))
sst_2 = sst_2_1 - sst_2_0
X2_zone = [15, -10, 175, 360-130]
...
for imonth in [6, 7, 8]:
    ...
    X1_sstCorr = xr.DataArray(X1_sstCorr, coords=[sst['lat'], sst['lon']],
                            dims=['lat', 'lon'], name='sst_corr')
    sub_pic(ax1, title=f'({chr(ord('a')+i)}) Reg_{imonth}_SST onto X1', extent=[-180, 180, -50, 80],
            ...
            shading2_corr=X1_sstCorr, p_test_drawSet2={'N': TR_time[1]-TR_time[0]+1, 'alpha': 0.1, 'lw': 0.2, 'color': '#454545'}, edgedraw2=False, cb_draw2=True if i==2 else False,
...
plt.savefig(f'{PYFILE}/p3/pic/type1_因子发展.pdf', bbox_inches='tight')
plt.savefig(f'{PYFILE}/p3/pic/type1_因子发展.png', dpi=600, bbox_inches='tight')
print(np.corrcoef(X1['sst'][1:], X2['sst']))