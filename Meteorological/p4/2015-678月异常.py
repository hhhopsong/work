...
t2m = t2m.sel(time=slice('1961-01-01', '2023-01-31'))
...
#ax1
# 读取CN05.1逐日最高气温数据
t2m_clim = t2m.mean('year')  # 逐月气候态
t2m_ano = t2m - t2m_clim  # 逐月距平
t2m_ano = masked(t2m_ano, fr"{PYFILE}/map/地图边界数据/长江区1：25万界线数据集（2002年）/长江区.shp")


fig = plt.figure(figsize=(6, 3))   # 创建画布

year = 2015
ax = fig.add_subplot(111, projection=proj)  # 添加子图
# 统一加粗所有四个边框
...
ax.set_title(f'Anom. t2m', fontsize=16, loc='left')

cmap = cmaps.GMT_polar[2:10] + cmaps.CBR_wet[0] + cmaps.GMT_polar[10:-2]
norm = colors.BoundaryNorm(boundaries=[-1.5, -1.0, -0.5, -0.2, 0.2, 1.0, 2.0, 3.0], ncolors=cmap.N)
a1 = ax.contourf(t2m['lon'], t2m['lat'], t2m_ano.sel(year=year).to_array()[0], cmap=cmaps.GMT_polar[2:10] + cmaps.CBR_wet[0] + cmaps.GMT_polar[10:-6], levels=[-1.5, -1.0, -0.5, -0.2, 0.2, 1.0], norm=norm, extend='both', transform=proj)
ax.add_geometries(Reader(fr'{PYFILE}/map/地图边界数据/长江区1：25万界线数据集（2002年）/长江区.shp').geometries(), ccrs.PlateCarree(), facecolor='none', edgecolor='black', linewidth=1)
...
ax_cbar = inset_axes(ax, width="3%", height="100%", loc='center right', bbox_to_anchor=(0.05, 0, 1, 1),
...
cb.ax.tick_params(length=0, labelsize=10, color='black', direction='in')#length为刻度线的长度
