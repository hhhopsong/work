...
# CN05.1 input file
CN05_NC_FILE = "/Volumes/TiPlus7100/data/CN05.1/CN05.1_Pre_1961_2023_daily.nc"
...
# Output figure
OUT_FIG = os.path.join(OUT_DIR, "Fig3_pre_daily_actual_CN051")
...
def plot_panel(
    ...
    ax.bar(
        ...
        alpha=0.45,
        linewidth=0,
        label=f"{TARGET_YEAR} daily precip.",
        zorder=2,
    )
    ...
    # 2015 cumulative precipitation: green line.
    ax2.plot(
        x,
        target_cum,
        color="green",
        linestyle="-",
        linewidth=1.5,
        label=f"{TARGET_YEAR} cumulative precip.",
        zorder=6,
    )

    # Climatological cumulative precipitation: black line.
    ax2.plot(
        x,
        clim_cum,
        color="black",
    ...
    cum_ymax = max(np.nanmax(target_cum), np.nanmax(clim_cum))
    ax2.set_ylim(0, cum_ymax * 1.15)

    ax.set_ylabel("Daily precipitation")
    ax2.set_ylabel("Cumulative precipitation")
...
def main():
    ...
    ax2 = plot_panel(
        ax,
        cn05_df,
        panel_title="Summer daily precipitation",
        dataset_name="CN05.1",
        show_legend=True,
    )
