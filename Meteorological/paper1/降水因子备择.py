...
def multi_core(num, m1, m2, ols, sen):
    ...
    pre_diff = xr.open_dataset(fr"cache\pre_diff\pre_{num}_{m1}_{m2}.nc")['pre'].transpose('lat', 'lon', 'time')  # 读取缓存
    try:
        ...
        np.save(fr"cache\corr_pre_1\corr_{num}_{m1}_{m2}.npy", corr_1)  # 保存缓存
    try:
        ...
        np.save(fr"cache\corr_pre_2\corr_{num}_{m1}_{m2}.npy", corr_2)
