...
def draw_frc():
    ...
    lev_range = np.linspace(-np.nanmax(np.abs(frc_nc_p[var].sel(lev=lev).data))/5, np.nanmax(np.abs(frc_nc_p[var].sel(lev=lev).data))/5, 8)
    ...
    t200_high = ax1.contour( lon_fill_white, lat, z_fill_white, levels=lev_Z[lev_Z > 0], cmap=cmaps.BlueDarkRed18[-1],
                        transform=ccrs.PlateCarree(central_longitude=0), linewidths=0.2)
    wind200 = Curlyquiver(ax1, lon_UV, lat, U, V, arrowsize=.3, scale=15, regrid=15, linewidth=0.4,
                        color='black', lon_trunc=-67.5, transform=ccrs.PlateCarree(central_longitude=0))

    # 图2
    lev = 500
    lev_Z = np.array([-10, -5, 5, 10]) * .01
    extent1 = extent1
    ...
    t500_high = ax2.contour( lon_fill_white, lat, z_fill_white, levels=lev_Z[lev_Z > 0], cmap=cmaps.BlueDarkRed18[-1],
                        transform=ccrs.PlateCarree(central_longitude=0), linewidths=0.2)
    wind500 = Curlyquiver(ax2, lon_UV, lat, U, V, arrowsize=.3, scale=15, regrid=15, linewidth=0.4,
                        color='black', lon_trunc=-67.5, transform=ccrs.PlateCarree(central_longitude=0))

    # 图1
    lev = 700
    lev_Z = np.array([-10, -5, 5, 10]) * .0025
    extent1 = extent1
    ...
    t700_high = ax3.contour( lon_fill_white, lat, z_fill_white, levels=lev_Z[lev_Z > 0], cmap=cmaps.BlueDarkRed18[-1],
                        transform=ccrs.PlateCarree(central_longitude=0), linewidths=0.2)
    wind850 = Curlyquiver(ax3, lon_UV, lat, U, V, arrowsize=.3, scale=15, regrid=15, linewidth=0.4,
                        color='black', lon_trunc=-67.5, transform=ccrs.PlateCarree(central_longitude=0))
    ...
    # 图1
    lev = 850
    lev_Z = np.array([-10, -5, 5, 10]) * 0.0025
    extent1 = extent1
    ax4 = fig.add_subplot(414, projection=ccrs.PlateCarree(central_longitude=180-67.5))
    ax4.set_title('d)Exp 850hPa UVZ', fontsize=6, loc='left')
    ...
    wind850 = Curlyquiver(ax4, lon_UV, lat, U, V, arrowsize=.3, scale=15, regrid=15, linewidth=0.4,
                        color='black', lon_trunc=-67.5, transform=ccrs.PlateCarree(central_longitude=0))
    DBATP = r"D:\PyFile\map\地图边界数据\青藏高原边界数据总集\TPBoundary_2500m\TPBoundary_2500m.shp"
    provinces = cfeature.ShapelyFeature(Reader(DBATP).geometries(), crs=ccrs.PlateCarree(), facecolor='gray', alpha=1)
    ...
    plt.savefig(r'D:\PyFile\pic\Output_热带对流.png', dpi=600, bbox_inches='tight')
    plt.show()
