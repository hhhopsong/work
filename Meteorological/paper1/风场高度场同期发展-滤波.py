...
def multi_core(var, p, ols, sen):
    ...
    if var == 'u' or var == 'v' or var == 'z':
        ...
        np.save(fr"D:\PyFile\paper1\cache\uvz\corr_{var[1]}{p}_same_imfs.npy", corr_1)
    elif var == 'sst':
...
if __name__ == '__main__':
    ...
    for date in [0]:
        ...
        for p in [150, 200, 500, 600, 700, 850]:
            if p < 200:
            ...
            if p == 200:
                ...
                velovect_key(fig, ax1, WAF图层, U=10, label='10 m$^2$/s$^2$', lr=-6.04, color='blue')
            ...
            if p == 150:
                ...
                ax.set_title('150hPa UVZ', fontsize=title_size, loc='left')
                ...
                WAF图层 = velovect(ax, z_diff['lon'], z_diff['lat'][:180],
                                  ...
                                  regrid=15, lon_trunc=-67.5, arrowsize=.3, scale=10, linewidth=0.4,
                                  color='blue', transform=ccrs.PlateCarree(central_longitude=0))
                velovect_key(fig, ax, WAF图层, U=10, label='10 m$^2$/s$^2$', lr=-6.04, color='blue')
                u_np = np.where(uv显著性检验结果 != 1, u_corr, np.nan)
                v_np = np.where(uv显著性检验结果 != 1, v_corr, np.nan)
                u_np = np.where(u_np**2 + v_np**2 >= 0.01**2, u_np, np.nan)
                v_np = np.where(u_np**2 + v_np**2 >= 0.01**2, v_np, np.nan)
