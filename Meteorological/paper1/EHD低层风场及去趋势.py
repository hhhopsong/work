...
# uvz
u_diff = xr.open_dataset(fr"D:\PyFile\paper1\cache\uvz\u_same.nc")['u'].sel(p=200).transpose('lat', 'lon', 'year')
u_corr = np.load(fr"D:\PyFile\paper1\cache\uvz\corr_u200_same.npy")
v_diff = xr.open_dataset(fr"D:\PyFile\paper1\cache\uvz\v_same.nc")['v'].sel(p=200).transpose('lat', 'lon', 'year')
v_corr = np.load(fr"D:\PyFile\paper1\cache\uvz\corr_v200_same.npy")
z_diff = xr.open_dataset(fr"D:\PyFile\paper1\cache\uvz\z_same.nc")['z'].sel(p=200).transpose('lat', 'lon', 'year')
z_corr = np.load(fr"D:\PyFile\paper1\cache\uvz\corr_z200_same.npy")
...
spec = gridspec.GridSpec(nrows=2, ncols=2, width_ratios=[1, 1], height_ratios=[1, 1])  # 设置子图比例

extent_changjiang = [90, 135, 18, 44]  # 中国大陆经度范围，纬度范围
extent_CN = [85, 145, 12, 48]  # 中国大陆经度范围，纬度范围
extent_CN = extent_changjiang
...
ax1.set_title('200hPa UVZ', fontsize=title_size, loc='left')
...
velovect_key(fig, ax1, uv_, U=1, label='1', lr=1, fontproperties={'size': 14}, linewidth=1.5)
...
# uvz
u_diff = xr.open_dataset(fr"D:\PyFile\paper1\cache\uvz\u_same.nc")['u'].sel(p=500).transpose('lat', 'lon', 'year')
u_corr = np.load(fr"D:\PyFile\paper1\cache\uvz\corr_u500_same.npy")
v_diff = xr.open_dataset(fr"D:\PyFile\paper1\cache\uvz\v_same.nc")['v'].sel(p=500).transpose('lat', 'lon', 'year')
v_corr = np.load(fr"D:\PyFile\paper1\cache\uvz\corr_v500_same.npy")
z_diff = xr.open_dataset(fr"D:\PyFile\paper1\cache\uvz\z_same.nc")['z'].sel(p=500).transpose('lat', 'lon', 'year')
z_corr = np.load(fr"D:\PyFile\paper1\cache\uvz\corr_z500_same.npy")
u显著性检验结果 = corr_test(ols, u_corr, alpha=alpha)
v显著性检验结果 = corr_test(ols, v_corr, alpha=alpha)
z显著性检验结果 = corr_test(ols, z_corr, alpha=alpha)
...
ax2.set_title('500hPa UVZ', fontsize=title_size, loc='left')
level = [0, 5, 10, 15, 20]  # 等值线间隔
custom_colors = ["#FDB57E", "#F26E4c", "#CA1E14", "#7F0000"]
custom_cmap = colors.ListedColormap(custom_colors)
...
level2 = [-.4, -.3, -.2, -.1, 0, .1, .2, .3, .4]
z_r = gaussian_filter(z_corr, 3)
a1_h = ax2.contour(z_lon, z_diff['lat'], z_r, transform=proj, levels=level2[:4], colors='blue', linewidths=1.5, linestyles='--',alpha=1)
a1_hm = ax2.contour(z_lon, z_diff['lat'], z_r, transform=proj, levels=[0], colors='gray', linewidths=1.5, linestyles='-', alpha=1)
a1_h1 = ax2.contour(z_lon, z_diff['lat'], z_r, transform=proj, levels=level2[5:], colors='red', linewidths=1.5, linestyles='-', alpha=1)
ax2.clabel(a1_h, inline=True, fontsize=14, fmt='%.01f', colors='blue', zorder=1)
ax2.clabel(a1_hm, inline=True, fontsize=14, fmt='%.01f', colors='gray', zorder=1)
ax2.clabel(a1_h1, inline=True, fontsize=14, fmt='%.01f', colors='red', zorder=1)
# ax2.text(122, 29.5, 'A', fontsize=20, fontweight='bold', color='blue', zorder=20)
####
uv_np_2 = velovect(ax2, u_diff['lon'], u_diff['lat'],
                  np.array(np.where(np.isnan(u_np), 0, u_np).tolist()),
                  np.array(np.where(np.isnan(v_np), 0, v_np).tolist()),
                  arrowsize=1, scale=5, linewidth=1, regrid=20,
                  color='gray', transform=ccrs.PlateCarree(central_longitude=0))
uv_2 = velovect(ax2, u_diff['lon'], u_diff['lat'],
               np.array(np.where(np.isnan(u_corr), 0, u_corr).tolist()),
               np.array(np.where(np.isnan(v_corr), 0, v_corr).tolist()),
               arrowsize=1, scale=5, linewidth=1, regrid=20,
               color='black', transform=ccrs.PlateCarree(central_longitude=0))
velovect_key(fig, ax2, uv_2, U=.5, label='0.5',lr=1, fontproperties={'size': 14}, linewidth=1.5)
...
ax2.add_geometries(Reader(r'D:\PyFile\map\地图线路数据\长江\长江.shp').geometries(),
                   ccrs.PlateCarree(), facecolor='none', edgecolor='blue', linewidth=0.2)
'''ax2.add_geometries(Reader(r'D:\PyFile\map\地图边界数据\青藏高原边界数据总集\TPBoundary_2500m\TPBoundary_2500m.shp').geometries(),
                   ccrs.PlateCarree(), facecolor='gray', edgecolor='gray', linewidth=.1, hatch='.', zorder=2)'''
# ax1.add_feature(provinces, lw=0.5, zorder=2)
# 设置坐标轴
xticks1=np.arange(extent_CN[0], extent_CN[1]+1, 10)
...
ax3.set_title('850hPa UVZ', fontsize=title_size, loc='left')
level = [0, 5, 10, 15, 20]  # 等值线间隔
custom_colors = ["#FDB57E", "#F26E4c", "#CA1E14", "#7F0000"]
custom_cmap = colors.ListedColormap(custom_colors)
...
level2 = [-.4, -.3, -.2, -.1, 0, .1, .2, .3, .4]
z_r = gaussian_filter(z_corr, 3)
a1_h = ax3.contour(z_lon, z_diff['lat'], z_r, transform=proj, levels=level2[:4], colors='blue', linewidths=1.5, linestyles='--',alpha=1)
a1_hm = ax3.contour(z_lon, z_diff['lat'], z_r, transform=proj, levels=[0], colors='gray', linewidths=1.5, linestyles='-', alpha=1)
a1_h1 = ax3.contour(z_lon, z_diff['lat'], z_r, transform=proj, levels=level2[5:], colors='red', linewidths=1.5, linestyles='-', alpha=1)
ax3.clabel(a1_h, inline=True, fontsize=14, fmt='%.01f', colors='blue', zorder=1)
ax3.clabel(a1_hm, inline=True, fontsize=14, fmt='%.01f', colors='gray', zorder=1)
ax3.clabel(a1_h1, inline=True, fontsize=14, fmt='%.01f', colors='red', zorder=1)
# ax3.text(122, 29.5, 'A', fontsize=20, fontweight='bold', color='blue', zorder=20)
####
uv_np_3 = velovect(ax3, u_diff['lon'], u_diff['lat'],
                  np.array(np.where(np.isnan(u_np), 0, u_np).tolist()),
                  np.array(np.where(np.isnan(v_np), 0, v_np).tolist()),
                  arrowsize=1, scale=5, linewidth=1, regrid=20,
                  color='gray', transform=ccrs.PlateCarree(central_longitude=0))
uv_3 = velovect(ax3, u_diff['lon'], u_diff['lat'],
               np.array(np.where(np.isnan(u_corr), 0, u_corr).tolist()),
               np.array(np.where(np.isnan(v_corr), 0, v_corr).tolist()),
               arrowsize=1, scale=5, linewidth=1, regrid=20,
               color='black', transform=ccrs.PlateCarree(central_longitude=0))
velovect_key(fig, ax3, uv_3, U=.25, label='0.25', lr=1, fontproperties={'size': 14}, linewidth=1.5)
...
sns.set(style='ticks')
ax3_pc = fig.add_axes(ax3.get_position())
# 设置坐标轴
ax3_pc_xmajor = FixedLocator([-1 + i*5 for i in range(13)]) #先定义xmajorLocator，再进行调用
ax3_pc_xminor = MultipleLocator(1)
ax3_pc.xaxis.set_major_locator(ax3_pc_xmajor)  #x轴最大刻度
ax3_pc.xaxis.set_minor_locator(ax3_pc_xminor)  #x轴最小刻度
ax3_pc_ymajor = MultipleLocator(.1)  #先定义xmajorLocator，再进行调用
ax3_pc_yminor = MultipleLocator(.02)
ax3_pc.yaxis.set_major_locator(ax3_pc_ymajor)  #x轴最大刻度
ax3_pc.yaxis.set_minor_locator(ax3_pc_yminor)  #x轴最小刻度
# 画条形图,正值为红色，负值为蓝色
a3_pc = sns.barplot(x=[i for i in range(1961, 2023)], y=ols, ax=ax3_pc)
for i in range(62):
    if ols[i] > 0:
        a3_pc.get_children()[i].set_color('#D85F4F')
    elif ols[i] == 0:
        a3_pc.get_children()[i].set_color('#F7F7F7')
    else:
        a3_pc.get_children()[i].set_color('#1F6AA0')
ax3_pc.set_xlim(-.5, 61.5)
ax3_pc.set_ylim(0, .4)
ax3_pc.set_yticks([0, .1, .2, .3, .4])
ax3_pc.set_yticklabels(['0', '10%', '20%', '30%', '40%'], fontsize=10)
#设定子图ax2大小位置
adjust_sub_axes(ax3, ax3_pc, shrink=1, lr=-.217, ud=1.0)
