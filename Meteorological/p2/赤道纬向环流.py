...
for i in range(len(K_type['type'])):
    ...
    f_ax.fill_between(Pre_nc['lon'], 125, 100, where=Pre_nc['reg'].mean('lat', skipna=True) >= 0, facecolor='green', zorder=9) # 降水异常
    f_ax.fill_between(Pre_nc['lon'], 125, 100, where=Pre_nc['reg'].mean('lat', skipna=True) <= 0, facecolor='#cca300', zorder=9) # 降水异常
