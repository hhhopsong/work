...
# PRE
pre = xr.open_dataset(fr'{DATA}/NOAA/GPCP/precip.mon.mean.nc').sel(time=slice('1961-01-01', '2023-12-31'))
...
for i in range(3):
    ...
    ax.set_title(f"({chr(ord('a') + i*2)}) {year[i]} EHTBW_days_anomaly", loc='left', fontsize=8)
    ...
    con = ax.contourf(CN051_2['lon'], CN051_2['lat'], EHTBW.sel(year=f'{year[i]}').sum(dim=['day'])-EHTBW_clim,
                      cmap=custom_cmap, transform=ccrs.PlateCarree(),
                      levels=lev, extend='max', norm=norm)
    ax.contour(CN051_2['lon'], CN051_2['lat'], EHTBW.sel(year=f'{year[i]}').sum(dim=['day'])-EHTBW_clim,
    ...
    cb1.set_ticklabels([f"{i:.0f}" for i in lev])
