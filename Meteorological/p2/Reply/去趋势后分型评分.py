...
try:
    ...
    del Tmax
T_th = 0.90
Tmax_5Day_filt = Tmax_5Day_filt.sel(day=slice('1', '88'))

# 将 year 坐标转为数值数组（例如 '1961' -> 1961）
year_nums = np.array([int(y) for y in Tmax_5Day_filt['year'].values])

# def detrend_1d(y, x):
#     # y: 一维数组，对应某个 (day,lat,lon) 在不同 year 上的值
#     mask = np.isfinite(y)
#     if mask.sum() < 2:
#         return y  # 数据不足或全 NaN，返回原数组
#     p = np.polyfit(x[mask], y[mask], 1)  # 线性拟合
#     trend = np.polyval(p, x)
#     return y - trend  # 返回去趋势后的序列
#
# # 对 year 维度应用去趋势函数，保留其他维度 (day, lat, lon)
# Tmax_5Day_filt = xr.apply_ufunc(
#     detrend_1d,
#     Tmax_5Day_filt,
...
#     kwargs={'x': year_nums},
#     dask='parallelized',
#     output_dtypes=[Tmax_5Day_filt.dtype]
# )
# 1) 计算流域逐年平均（对 day, lat, lon 求平均）
basin_year_mean = Tmax_5Day_filt.mean(dim=['day', 'lat', 'lon'])

# 2) 用非空值拟合线性趋势
mask = np.isfinite(basin_year_mean.values)
if mask.sum() >= 2:
    p = np.polyfit(year_nums[mask], basin_year_mean.values[mask], 1)  # 线性拟合
    trend_year = np.polyval(p, year_nums)
else:
    trend_year = np.zeros_like(year_nums, dtype=float)

# 3) 构造与原数据在 year 维度对齐的 DataArray（xarray 会自动广播到 day, lat, lon）
trend_da = xr.DataArray(trend_year, coords={'year': Tmax_5Day_filt['year']}, dims=['year'])

# 4) 从每个格点的时间序列中减去流域年趋势（保留原有维度和坐标）
Tmax_5Day_filt = Tmax_5Day_filt - trend_da


Tmax_5Day_filt = Tmax_5Day_filt.transpose('year', 'day', 'lat', 'lon')
t95 = masked(Tmax_5Day_filt,
...
def plot_test(data, max_clusters=10):
    ...
    fig.tight_layout()
    plt.savefig(fr"{PYFILE}/p2/pic/reply/fig_r3_1.pdf", bbox_inches='tight')
    plt.savefig(fr"{PYFILE}/p2/pic/reply/fig_r3_1.png", dpi=600, bbox_inches='tight')
...
lev = np.array([[0, 800, 900, 1000, 1100, 1110, 1120, 1125, 1130],
                [0, 500,  600,  700, 750,  800,  815,  820,  825],
                [0, 170, 190,  210,  230,  250,  260,  270,  280]])
...
Tavg_weight.to_netcdf(fr"{PYFILE}/p2/data/Tavg_weight_reply_detrend.nc")
...
try:
    Time_type.to_netcdf(fr'{PYFILE}/p2/data/Time_type_AverFiltAll_reply.nc')
except:
    pass
...
# 加载数据集
data = xr.open_dataset(fr'{PYFILE}/p2/data/Time_type_AverFiltAll_reply.nc')
data = Time_type

#  0.95892163, -0.3483075 , -0.17019883
# -0.23556069,  0.9404109 ,  0.01598896
# -0.22251315,  0.02862138,  0.94476446
...
# 显示图表
plt.savefig(fr"{PYFILE}/p2/pic/reply/fig_r2.pdf", bbox_inches='tight')
plt.savefig(fr"{PYFILE}/p2/pic/reply/fig_r2.png", dpi=600, bbox_inches='tight')
