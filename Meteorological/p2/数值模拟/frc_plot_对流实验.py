...
def draw_frc():
    ...
    var_contr = ax1.contourf(lon_fill_white, frc_nc_p[var]['lat'], np.where(frc_fill_white != 0, frc_fill_white, np.nan),
                        levels=lev_range, cmap=cmaps.BlueWhiteOrangeRed[20:-20], transform=ccrs.PlateCarree(central_longitude=0), extend='both')
    wind200 = Curlyquiver(ax1, lon_UV, lat, U, V, arrowsize=.3, scale=5, regrid=13, linewidth=.25,
                        color='dimgray', center_lon=180-70)
    wind200.key(fig, U=.2, label='0.2 m/s', ud=7.8, edgecolor='none', arrowsize=.5, color='k')
    # 图2
    lev = 500
    extent1 = extent1
    ...
    for collection in t500_high.collections:
        collection.set_edgecolor('#ef3b2b')  # -----打点颜色设置
    var_contr = ax2.contourf(lon_fill_white, frc_nc_p[var]['lat'], np.where(frc_fill_white != 0, frc_fill_white, np.nan),
                        levels=lev_range, cmap=cmaps.BlueWhiteOrangeRed[20:-20], transform=ccrs.PlateCarree(central_longitude=0), extend='both')
    wind500 = Curlyquiver(ax2, lon_UV, lat, U, V, arrowsize=.3, scale=5, regrid=13, linewidth=.25,
                        color='dimgray', center_lon=180-70, nanmax=wind200.nanmax)
    wind500.key(fig, U=.2, label='0.2 m/s', ud=7.8, edgecolor='none', arrowsize=.5, color='k')
    ...
    for collection in t800_high.collections:
        collection.set_edgecolor('#ef3b2b')  # -----打点颜色设置
    var_contr = ax4.contourf(lon_fill_white, frc_nc_p[var]['lat'], np.where(frc_fill_white != 0, frc_fill_white, np.nan),
                        levels=lev_range, cmap=cmaps.BlueWhiteOrangeRed[20:-20], transform=ccrs.PlateCarree(central_longitude=0), extend='both')
    #z850 = ax4.contour(lon_Z, lat, Z, levels=4, colors='black', transform=ccrs.PlateCarree(central_longitude=0), linewidths=0.4)
    wind850 = Curlyquiver(ax4, lon_UV, lat, U, V, arrowsize=.3, scale=5, regrid=13, linewidth=.25,
                        color='dimgray', center_lon=180-70, nanmax=wind200.nanmax)
    wind850.key(fig, U=.2, label='0.2 m/s', ud=7.8, edgecolor='none', arrowsize=.5, color='k')
    ...
    plt.savefig(r'D:\PyFile\p2\pic\Output_对流实验.pdf', dpi=600, bbox_inches='tight')
    plt.show()
