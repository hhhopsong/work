...
def draw_frc():
    ...
    t200_high = ax1.contour( lon_fill_white, lat, z_fill_white, levels=lev_Z[lev_Z > 0], cmap=cmaps.BlueDarkRed18[-1],
                        transform=ccrs.PlateCarree(central_longitude=0), linewidths=0.2)'''
    wind200 = Curlyquiver(ax1, lon_UV, lat, U, V, arrowsize=.12, scale=20, regrid=25, linewidth=0.15,
                        color='black', center_lon=180-70)
    wind200.key(fig, U=.3, label='0.3 m/s', ud=7.8, edgecolor='none', arrowsize=.5, color='k')
    # 图2
    lev = 500
    lev_Z = np.array([-10, -5, 5, 10]) * 2
    extent1 = extent1
    ...
    t500_high = ax2.contour( lon_fill_white, lat, z_fill_white, levels=lev_Z[lev_Z > 0], cmap=cmaps.BlueDarkRed18[-1],
                        transform=ccrs.PlateCarree(central_longitude=0), linewidths=0.2)'''
    wind500 = Curlyquiver(ax2, lon_UV, lat, U, V, arrowsize=.12, scale=15, regrid=25, linewidth=0.15,
                        color='black', center_lon=180-70)
    wind500.key(fig, U=.3, label='0.3 m/s', ud=7.8, edgecolor='none', arrowsize=.5, color='k')
    ...
    wind850 = Curlyquiver(ax4, lon_UV, lat, U, V, arrowsize=.12, scale=10, regrid=25, linewidth=0.15,
                        color='black', center_lon=180-70)
    wind850.key(fig, U=.3, label='0.3 m/s', ud=7.8, edgecolor='none', arrowsize=.5, color='k')
    DBATP = r"D:\PyFile\map\地图边界数据\青藏高原边界数据总集\TPBoundary_2500m\TPBoundary_2500m.shp"
    provinces = cfeature.ShapelyFeature(Reader(DBATP).geometries(), crs=ccrs.PlateCarree(), facecolor='gray', alpha=1)
