...
MANUAL_FACTORS = [
    ...
    ('X5_19', 'swvl',[5, 6], None,  50,  68,  0,  40),
]
...
index_N = 100  # 最终最多保留的因子个数（可按需调整）

# 把所有手动因子整理成统一的 dict
all_X_train, all_X_pre, all_X_rollingCorr = {}, {}, {}
for k in X_train_all_dict.keys():
    all_X_train[k] = X_train_all_dict[k].rename(k)
    all_X_pre[k] = X_pre_all_dict[k].rename(k)
    all_X_rollingCorr[k] = X_roll_all_dict[k].rename(k)
...
# 直接把全部手动因子放入后退逐步回归
df_step = df_train_full[['TS'] + candidate_predictors] \
    .replace([np.inf, -np.inf], np.nan).dropna().copy()

print("Training rows used for stepwise regression:", len(df_step))

step_model, model_predictors = stepwise_regression(
    data=df_step,
    Dep='TS',
    InDep=candidate_predictors,
    limit=0.05
)

if len(model_predictors) == 0:
    # 回退：按训练期 |corr| 取前 index_N 个
    corr_rank = {}
    for x in candidate_predictors:
        sub = df_step[['TS', x]].dropna()
        if len(sub) >= 10:
            corr_rank[x] = abs(sub['TS'].corr(sub[x]))

    model_predictors = sorted(corr_rank, key=corr_rank.get, reverse=True)[:index_N]
    print("Stepwise returned empty. Fallback predictors:", model_predictors)

# 若后退逐步回归保留变量多于 index_N，则按最终模型中 |t| 最大的前 index_N 个保留
if len(model_predictors) > index_N:
    df_tmp = df_step[['TS'] + model_predictors].dropna().copy()
    x_tmp = sm.add_constant(df_tmp[model_predictors], has_constant='add')
    y_tmp = df_tmp['TS']
    tmp_model = sm.OLS(y_tmp, x_tmp).fit()

    tvals = tmp_model.tvalues.copy()
    tvals.drop('const', inplace=True, errors='ignore')
    tvals.drop('Intercept', inplace=True, errors='ignore')

    model_predictors = tvals.abs().sort_values(ascending=False).index[:index_N].tolist()
    print(f"[Final-limit] keep top {index_N} by |t|:", model_predictors)

model_predictors = model_predictors[:index_N]
print("Selected predictors:", model_predictors)

plot_predictors = candidate_predictors.copy()      # 全部手动因子都画
...
# 9.1 区域图（红=入选，蓝虚线=剔除）
# ------------------------------------------------------------
region_fig_dir = fr'{PYFILE}/p5/pic/manual_predictor_region_maps'

saved_region_files = plot_manual_predictor_region_maps(
    meta_df=meta_df,
    timeSerie=timeSerie,
    TR_time=TR_time,
    PR_time=PR_time,
...
late_sig_predictors = meta_df.loc[meta_df['late_sig_same_sign'] == True, 'X_name'].tolist()
...
for xname in plot_predictors:
    is_selected = xname in model_predictors
    is_late_sig = xname in late_sig_predictors

    if is_selected:
        color, lw, alpha, zorder = 'red', 1.8, 1.0, 3
    elif is_late_sig:
        color, lw, alpha, zorder = 'orange', 1.4, 0.95, 2
    else:
        color, lw, alpha, zorder = '#959595', 0.9, 0.75, 1
...
# 9.3 最终回归建模
# ------------------------------------------------------------
df_model_train = df_train[['TS'] + selected_predictors] \
    .replace([np.inf, -np.inf], np.nan).dropna().copy()

if len(model_predictors) == 0:
    raise ValueError("No predictors selected. Please check candidate predictors or p-value limit.")
...
ax_predict.axvline(
    x=pd.to_datetime(f'{TR_time[1]}-6-30'),
    color='orange',
    linestyle='-',
    linewidth=1
)
...
tcc_text_train = (
    f"TCC={train_eval['TS'].corr(train_eval['predicted_TS']):.2f}"
    if len(train_eval) > 1 else "TCC=nan"
)

rmse_text_train = (
    f"RMSE={np.sqrt(np.mean((train_eval['TS'] - train_eval['predicted_TS']) ** 2)):.2f}"
    if len(train_eval) > 0 else "RMSE=nan"
)
...
tcc_text_pre = (
    f"TCC={pre_eval['TS'].corr(pre_eval['inDependent_pre']):.2f}"
    if len(pre_eval) > 1 else "TCC=nan"
)

rmse_text_pre = (
    f"RMSE={np.sqrt(np.mean((pre_eval['TS'] - pre_eval['inDependent_pre']) ** 2)):.2f}"
    if len(pre_eval) > 0 else "RMSE=nan"
)
...
plt.savefig(fr'{PYFILE}/p5/pic/pemodle_manual.pdf', bbox_inches='tight')
plt.savefig(fr'{PYFILE}/p5/pic/pemodle_manual.png', dpi=600, bbox_inches='tight')
plt.close(fig)
