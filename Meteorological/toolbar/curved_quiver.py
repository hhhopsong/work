...
def curved_quiver(ax, X, Y, U, V, color='k',scaling=False,length_scale=0, scale=30, minlength=.95, maxlength=1.0, integration_direction='backward', density=10, arrowsize=0.0):
    ...
    if not scaling:
        for i in range(start_points.shape[0]):
            ax.streamplot(X,Y,U,V, color=color, start_points=np.array([start_points[i,:]]),minlength=minlength*norm_flat[i]*scale_, maxlength=maxlength*norm_flat[i]*scale_,
                        integration_direction=integration_direction, density=density, arrowsize=arrowsize)
        ...
        for i in range(start_points.shape[0]):
            ax.streamplot(X,Y,U,V, color=color, start_points=np.array([start_points[i,:]]),minlength=.95*norm_flat[i]*scale_, maxlength=1.0*norm_flat[i]*scale_,
                        integration_direction=integration_direction, density=density, arrowsize=arrowsize, linewidth=.5*norm_flat[i])
